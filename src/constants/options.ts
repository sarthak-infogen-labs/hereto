export const aiFrontierOptions = [
  { id: "agentic-ai", label: "Agentic AI", value: "agentic-ai" },
  {
    id: "agent2agent-protocol-a2a",
    label: "Agent2Agent Protocol (A2A)",
    value: "agent2agent-protocol-a2a",
  },
  {
    id: "model-context-protocol-mcp",
    label: "Model Context Protocol (MCP)",
    value: "model-context-protocol-mcp",
  },
  {
    id: "retrieval-augmented-generation-rag",
    label: "Retrieval-Augmented Generation (RAG)",
    value: "retrieval-augmented-generation-rag",
  },
  {
    id: "intelligent-process-automation",
    label: "Intelligent Process Automation (IPA)",
    value: "intelligent-process-automation",
  },
];

export const solutionsOptions = [
  { id: "ai-ml", label: "Artificial Intelligence & ML", value: "ai-ml" },
  {
    id: "data-driven-transformation",
    label: " Data-Driven Transformation",
    value: "data-driven-transformation",
  },
  {
    id: "product-engineering",
    label: "Product Engineering",
    value: "product-engineering",
  },
  {
    id: "next-gen-transformation",
    label: "Next-Gen Transformation",
    value: "next-gen-transformation",
  },
];

export const aiOptions = [
  { id: "ai-agents", label: "AI Agents", value: "ai-agents" },
  { id: "mcp", label: "MCP & A2A", value: "mcp" },
  { id: "rag", label: "RAG", value: "rag" },
  { id: "ipa", label: "IPA", value: "ipa" },
];
